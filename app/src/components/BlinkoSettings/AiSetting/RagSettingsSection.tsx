...
// CUSTOM-JOURNAL: admin-only tab for the RAG (vector search) subsystem --
// basic index stats direct from the vector store, toggles for which
// per-note metadata (dates/mood/tags) gets prepended to the text actually
// embedded (see AiService.embedNoteWithMetadata), and a history log of
// every embedding action (reusing aiTaskLog, taskType 'embedding', the same
// log every other AI action already writes to). Only rendered for
// superadmins -- see AiSetting.tsx.
export const RagSettingsSection = observer(function RagSettingsSection() {
  ...
  const [ragIncludeDates, setRagIncludeDates] = useState(false);
  const [ragIncludeMood, setRagIncludeMood] = useState(false);
  const [ragIncludeTags, setRagIncludeTags] = useState(false);
  ...
  const fetchPage = async (targetPage: number, reset: boolean) => {
    setLoading(true);
    try {
      const rows = await api.ai.aiTaskLogList.query({ page: targetPage, size: PAGE_SIZE, scope: 'all', taskType: 'embedding' });
  ...
  useEffect(() => {
    if (blinko.config.value) {
      setRagIncludeDates(!!blinko.config.value.ragIncludeDates);
      setRagIncludeMood(!!blinko.config.value.ragIncludeMood);
      setRagIncludeTags(!!blinko.config.value.ragIncludeTags);
    }
  }, [blinko.config.value]);

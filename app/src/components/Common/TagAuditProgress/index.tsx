...
// CUSTOM-JOURNAL: cloned from Common/RebuildEmbeddingProgress, same
// start/poll/stop shape, pointed at the tagAudit* procedures instead.
export const TagAuditProgress = observer(({ force }: { force: boolean }) => {
  ...
  const store = RootStore.Local(() => ({
    ...
    async fetchProgress() {
      try {
        const result = await api.ai.tagAuditProgress.query();
        if (result) {
        ...
        console.error("Error fetching tag audit progress:", err);
    ...
    async handleStart() {
      try {
        await api.ai.tagAuditStart.mutate({ force });

        store.startPolling();

        store.message.unshift({
          type: 'info',
          content: t('tag-audit-started'),
        });

        blinko.updateTicker++;
      } catch (err) {
        RootStore.Get(ToastPlugin).error(err?.message || "Failed to start tag audit task");
      }
    },

    async stopTask() {
      try {
        await api.ai.tagAuditStop.mutate();
        ...
        store.message.unshift({
          type: 'info',
          content: t('tag-audit-stopped-by-user'),
        });
        ...
        RootStore.Get(ToastPlugin).error(err?.message || "Failed to stop tag audit task");
      }
    }
  }))
    ...
    <div className="space-y-4">
      <div className="space-y-3">
        <Progress
          ...
          label={<span>
            <span className="font-medium">{store.progress}</span> / <span className="font-medium">{store.total}</span> {t('entries')}
          </span>}
          radius="none"
          showValueLabel={true}
          size="sm"
...
export const ShowTagAuditProgressDialog = async (force = false) => {
  RootStore.Get(DialogStandaloneStore).setData({
    title: i18n.t('tag-audit-progress'),
    content: <TagAuditProgress force={force} />,
    isOpen: true,
    size: 'lg',
  })
}
